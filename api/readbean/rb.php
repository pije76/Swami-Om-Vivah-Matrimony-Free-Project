...
class RedBean_Driver_PDO implements RedBean_Driver {
    ...
    public function connect() {
        ...
        try {
            ...
            $this->pdo = new PDO(
                    $this->dsn, $user, $pass, array(1002 => 'SET NAMES utf8',
...
class RedBean_QueryWriter_SQLiteT extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter {
    ...
    public function scanType($value, $flagSpecial = false) {
        ...
        if (is_numeric($value) && (intval($value) == $value) && $value < 2147483648)
            return self::C_DATATYPE_INTEGER;
        if ((is_numeric($value) && $value < 2147483648) || preg_match('/\d{4}\-\d\d\-\d\d/', $value) || preg_match('/\d{4}\-\d\d\-\d\d\s\d\d:\d\d:\d\d/', $value)
        ) {
    ...
    protected function buildFK($type, $targetType, $field, $targetField, $constraint = false) {
        ...
        if (isset($t['keys'][$label]) && $t['keys'][$label]['table'] === $targetType && $t['keys'][$label]['from'] === $field && $t['keys'][$label]['to'] === $targetField && $t['keys'][$label]['on_delete'] === $consSQL
        )
            return false;
        $t['keys'][$label] = array(
            'table' => $targetType,
...
class RedBean_QueryWriter_PostgreSQL extends RedBean_QueryWriter_AQueryWriter implements RedBean_QueryWriter {
    ...
    public function scanType($value, $flagSpecial = false) {
        ...
        if ($sz)
            return self::C_DATATYPE_TEXT;
        if ($value === null || ($value instanceof RedBean_Driver_PDO_NULL) || (is_numeric($value) && floor($value) == $value && $value < 2147483648 && $value > -2147483648)) {
            return self::C_DATATYPE_INTEGER;
...
class RedBean_OODB extends RedBean_Observable {
    ...
    public function find($type, $conditions = array(), $addSQL = null, $all = false) {
        try {
            ...
            if (!$this->writer->sqlStateIn($e->getSQLState(), array(
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE,
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN)
                    ))
                throw $e;
    ...
    private function processEmbeddedBeans($bean, $embeddedBeans) {
        foreach ($embeddedBeans as $linkField => $embeddedBean) {
            if (!$this->isFrozen) {
                $this->writer->addIndex($bean->getMeta('type'), 'index_foreignkey_' . $bean->getMeta('type') . '_' . $embeddedBean->getMeta('type'), $linkField);
    ...
    private function processAdditions($bean, $ownAdditions) {
        ...
        foreach ($ownAdditions as $addition) {
            if ($addition instanceof RedBean_OODBBean) {
                ...
                $this->store($addition);
                if (!$this->isFrozen) {
                    $this->writer->addIndex($addition->getMeta('type'), 'index_foreignkey_' . $addition->getMeta('type') . '_' . $bean->getMeta('type'), $myFieldLink);
    ...
    public function load($type, $id) {
        ...
        if (isset($this->stash[$this->nesting][$id])) {
            ...
            try {
                ...
                if (
                        $this->writer->sqlStateIn($e->getSQLState(), array(
                            RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                            RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE)
                        )
                ) {
    ...
    public function trash($bean) {
        ...
        if (!$this->isFrozen)
            $this->check($bean);
        try {
            $this->writer->selectRecord($bean->getMeta('type'), array('id' => array($bean->id)), null, true);
        } catch (RedBean_Exception_SQL $e) {
            if (!$this->writer->sqlStateIn($e->getSQLState(), array(
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                        RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE)
...
class RedBean_AssociationManager extends RedBean_Observable {
    ...
    public function relatedSimple($bean, $type, $sql = null, $values = array()) {
        ...
        if (count($keys) == 0 || !is_array($keys))
            return array();
        if (!$sql)
            return $this->oodb->batch($type, $keys);
    ...
    private static function query($method, $sql, $values) {
        if (!self::$redbean->isFrozen()) {
            try {
                ...
                if (self::$writer->sqlStateIn($e->getSQLState(), array(
                            RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_COLUMN,
                            RedBean_QueryWriter::C_SQLSTATE_NO_SUCH_TABLE)
                        )) {
